...
import { TbAntennaBars4, TbAntennaBars5, TbAntennaBars1 } from "react-icons/tb";
    ...
    <div className="w-[100%]">
      ...
      <div className="flex flex-col gap-4">
        {tickets.map((ticket) => (
          <div
            key={ticket.id}
            className="border p-4 rounded shadow-md bg-white flex flex-col gap-3 dark:bg-[#161B22] dark:border-[#5d636a] "
          >
              ...
              {selectedGroup !== "user" ? (
                <>
                  ...
                  <div className="relative">
                    <img
                      src={`https://api.dicebear.com/5.x/initials/svg?seed=${getUserInfo(
                        ticket.userId
                      ).name}`}
                      alt=""
                      className="h-4 w-4 rounded-full z-0"
                    />
                    {getUserInfo(ticket.userId).available ? (
                      <div className="h-[6px] w-[6px] rounded-full bg-orange-300 border border-white absolute right-0 bottom-2 z-10"></div>
                    ) : (
                      <div className="h-2 w-2 rounded-full bg-slate-600 border border-white absolute right-0 bottom-2 z-10"></div>
                    )}
                  </div>
                </>
              ) : (
            ...
            <h3 className="text-[16px] text-black font-medium leading-4 flex flex-row gap-1 dark:text-white">
              {selectedGroup === "user" || selectedGroup === "priority" ? (
